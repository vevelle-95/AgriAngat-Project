...
def get_weather_api(city_name):
    ...
    try:
        ...
        if forecast_response.status_code == 200:
            ...
            # Convert to forecast format (include today, next 2 days)
            dates = sorted(daily_forecasts.keys())[:3]  # Include today, get next 3 days total
...
def get_local_ip_addresses():
    ...
    try:
        # Get primary local IP (most reliable method)
...
def print_server_info():
    """Print server startup information with all available IPs"""
    ...
    for ip in ip_addresses:
        ...
        print(f"  http://{ip}:5000/api/weather/<city>")
    
    print("\n📊 Using OpenWeatherMap API for real weather data")
    print("🔗 Mobile app can connect to any of the above URLs")
    print("=" * 70)
